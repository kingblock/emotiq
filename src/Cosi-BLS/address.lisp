...
;;; friendly, i.e., shorter, with characters easily typed and read and
;;; distinguishable by humans in nearly all computing environments, and with at
;;; least simple checksum-type checking available, and with a version prefix.
;;; To achieve this, we can adopt the Bitcoin specification for addresses.  For
;;; the shortening, we use repemd160 as a second to cut the number of bits down
;;; to 160 bits. As the first hash we can choose to use either sha-2/256 (for
;;; Bitcoin identicality) or sha-3/256 (for uniformity with our hashing
;;; elsewhere).
...
(defparameter *mainnet-version-for-public-key-to-address* 234
  "An byte octet thats serves as a version prefix for mainnet public key hash
   address creation.")

(defparameter *testnet-version-for-public-key-to-address* 235
  "An byte octet thats serves as a version prefix for testnet public key hash
   address creation.")

;; NOTE: the mainnet version is the preferred/default version below. Is that
;; right? -mhd, 5/26/18
...
(defun octet-vector-to-integer-value (octet-vector)
  (loop with integer-value = 0
        ...
        ;; for i from 0 below (length octet-vector)
        as j from 0
...
(defun public-key-to-address (public-key &key net override-version)
  ...
  (let* ((sha2/256 (print (hash:hash/sha2/256 public-key)))
         ...
         (prefix+data
           (print (concatenate 'vector prefix-vec data-vec)))
         (checksum-vec
         ...
         (prefix+data+checksum (concatenate 'vector prefix+data checksum-vec))
         ;; encode that in base 58
         (base58-encoded-prefix+data+checksum (b58 prefix+data+checksum)))
    base58-encoded-prefix+data+checksum))
